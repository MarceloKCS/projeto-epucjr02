				...
				<select name="Congregacao">
					...
					if(formularioDeObreiro != null && formularioDeObreiro.verificarCampoPreenchido("congregacao")){
						for(int posicao=1; posicao<=listaDeCongregacao.size(); posicao++){
							...
							if(congregacao.equals(listaDeCongregacao.get(posicao))){
								out.println("<option selected=\"selected\" value=" + listaDeCongregacao.get(posicao) +">"+listaDeCongregacao.get(posicao)+"</option>");
							}
							else{
								out.println("<option value=\""+listaDeCongregacao.get(posicao)+"\">"+listaDeCongregacao.get(posicao)+"\"</option>");
							}
						}
					}
					else{
